...
class Vehicle: NSObject {
    ...
    init(maxFuel: UInt) {
        ...
        self.remainingFuel = Int(maxFuel)
    }
    
    func move(terrain: Terrain) throws {
        if remainingFuel - Int(terrain.fuelConsumption) < 0 {
            throw QuizError.OutOfFuel(fuel: terrain.fuelConsumption)
        }
        
        terrains.append(terrain)
        remainingFuel -= Int(terrain.fuelConsumption)
    }
}
